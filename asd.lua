...
local function processLemon(v, hrp)
    ...
    universalClick(vps.X / 2, vps.Y / 2)
    return v.Parent == nil
end
                ...
                if phone and phone.Enabled then
                    for _, d in ipairs_(phone:GetDescendants()) do
                        if d:IsA("TextButton") and d.Visible and (d.Text:lower():find("accept") or d.Text:lower():find("take")) then
                            ...
                            universalClick(apos.X + asz.X / 2, apos.Y + asz.Y / 2 + inset)
...
function RB.prepClick()
    pcall_(function()
        if lemonFarmActive or LSM.zoomedIn then
            LSM.zoomedIn = false
            for _ = 1, 10 do VirtualInputManager:SendMouseWheelEvent(0, 0, false, game) task_wait(0.01) end
        end
    end)
end
...
print("[Hub] Unified Mobile Automation Engine successfully loaded.")
